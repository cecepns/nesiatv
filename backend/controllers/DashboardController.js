...
const stats = async (req, res) => {
  try {
    const [mangaCount] = await db.execute('SELECT COUNT(*) as total FROM anime');
    const [categoryCount] = await db.execute('SELECT COUNT(*) as total FROM categories');
    const [viewsResult] = await db.execute('SELECT COALESCE(SUM(views), 0) as total FROM anime');
