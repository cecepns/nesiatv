...
const addItems = async (req, res) => {
  try {
    ...
    const animeIds = await resolveMangaIds(req.body || {});
    ...
    for (const animeId of animeIds) {
      const [ins] = await db.execute(
        'INSERT IGNORE INTO user_readlist_manga (readlist_id, anime_id) VALUES (?, ?)',
        [readlistId, animeId]
      );
...
const removeItem = async (req, res) => {
  try {
    ...
    await db.execute(
      'DELETE FROM user_readlist_manga WHERE readlist_id = ? AND anime_id = ?',
      [readlistId, animeId]
    );
