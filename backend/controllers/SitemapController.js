...
const DEFAULT_SITE_URL = 'https://id.nesiatv.net';
const DEFAULT_API_URL = 'https://api-be.nesiatv.my.id';
...
const SITEMAP_SITE_URL = 'https://id.nesiatv.net';
...
const robots = (req, res) => {
  ...
  const robotsTxt = `# robots.txt for Nesiatv API
# ${apiUrl}
...
const sitemapMain = async (req, res) => {
  try {
    ...
    for (const manga of mangaRows) {
      ...
      xml += '  <url>\n';
      xml += `    <loc>${siteUrl}/anime/${encodeURIComponent(manga.slug)}</loc>\n`;
    ...
    for (const chapter of chapterRows) {
      ...
      xml += '  <url>\n';
      xml += `    <loc>${siteUrl}/watch/${encodeURIComponent(chapter.slug)}</loc>\n`;
      xml += `    <lastmod>${lastmod}</lastmod>\n`;
