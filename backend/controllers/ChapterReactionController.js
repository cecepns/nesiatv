  ...
  try {
    ...
    if (req.user) {
      const [uv] = await db.execute(
        'SELECT reaction_type FROM episode_reactions WHERE episode_id = ? AND user_id = ?',
        [episodeId, req.user.id]
      );
      userReactionRow = uv.length > 0 ? uv[0] : null;
    } else {
      const [uv] = await db.execute(
        'SELECT reaction_type FROM episode_reactions WHERE episode_id = ? AND user_ip = ? AND (user_id IS NULL OR user_id = 0)',
        [episodeId, actorKey]
      );
      userReactionRow = uv.length > 0 ? uv[0] : null;
    }
  ...
  try {
    ...
    if (userId) {
      ...
      if (existing.length === 0) {
        const [byDevice] = await db.execute(
          `SELECT id, reaction_type, user_id
           FROM episode_reactions
           WHERE episode_id = ? AND user_ip = ? AND (user_id IS NULL OR user_id = 0)
           ORDER BY id ASC
           LIMIT 1`,
          [episodeId, actorKey]
        );
    ...
    if (existing.length > 0) {
      ...
      if (existingRow.reaction_type === reaction_type) {
        if (isOwnedByUser) {
          await db.execute('DELETE FROM episode_reactions WHERE id = ?', [existingRow.id]);
          return res.json({ status: true, message: 'Reaction removed', action: 'removed' });
        }

        if (userId && !isOwnedByUser) {
          await db.execute(
            'UPDATE episode_reactions SET user_id = ?, user_ip = ? WHERE id = ?',
      ...
      await db.execute(
        'UPDATE episode_reactions SET reaction_type = ?, user_id = ?, user_ip = ? WHERE id = ?',
    ...
    if (userId) {
      await db.execute(
        `INSERT INTO episode_reactions (episode_id, reaction_type, user_id, user_ip)
         VALUES (?, ?, ?, ?)`,
        [episodeId, reaction_type, userId, actorKey]
      );
    } else {
      await db.execute(
        `INSERT INTO episode_reactions (episode_id, reaction_type, user_ip)
         VALUES (?, ?, ?)`,
        [episodeId, reaction_type, actorKey]
      );
    }
