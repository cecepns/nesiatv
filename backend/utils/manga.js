...
async function fetchLocalManga(filters) {
  ...
  if (genreIds.length > 0) {
    query += ' INNER JOIN anime_genres mg ON m.id = mg.anime_id';
  }
  ...
  const animeIds = mangaRows.map((m) => m.id);

  let genresByMangaId = {};
  try {
    const genrePlaceholders = animeIds.map(() => '?').join(',');
    const [genreRows] = await db.execute(
      `
        ...
        WHERE mg.anime_id IN (${genrePlaceholders})
      `,
      animeIds
    );
  ...
  try {
    const chapterPlaceholders = animeIds.map(() => '?').join(',');
    const [lastChapterRows] = await db.execute(
      `
        ...
        FROM (
          ...
          FROM chapters
          WHERE anime_id IN (${chapterPlaceholders})
          GROUP BY anime_id
        ) t
        JOIN chapters c
          ON c.anime_id = t.anime_id
