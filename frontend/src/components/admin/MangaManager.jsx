...
const MangaManager = () => {
  ...
  const [draggedOverImage, setDraggedOverImage] = useState(null); // Track image being dragged over {episodeId, imageId}
  const [isReordering, setIsReordering] = useState({}); // Track reordering state per chapter

  const fetchManga = useCallback(async () => {
    try {
      setLoading(true);
      const response = await apiClient.getAnime(
        currentPage,
        10,
  ...
  const fetchChapters = async (animeId) => {
    try {
      const chaptersData = await apiClient.getEpisodes(animeId);
  ...
  const handleDeleteChapter = async (episodeId) => {
    if (!confirm("Apakah Anda yakin ingin menghapus chapter ini?")) return;

    try {
      await apiClient.deleteChapter(episodeId);
      await fetchChapters(selectedMangaForChapters.id);
    } catch (error) {
  ...
  const fetchChapterImages = async (episodeId) => {
    try {
      setLoadingImages((prev) => ({ ...prev, [episodeId]: true }));
      const images = await apiClient.getEpisodeVideos(episodeId);
      setChapterImagesMap((prev) => ({ ...prev, [episodeId]: images }));
    } catch (error) {
      console.error("Error fetching chapter images:", error);
      setChapterImagesMap((prev) => ({ ...prev, [episodeId]: [] }));
    } finally {
      setLoadingImages((prev) => ({ ...prev, [episodeId]: false }));
    }
  };

  const handleToggleChapterImages = async (episodeId) => {
    const isExpanded = expandedChapters[episodeId];
    setExpandedChapters((prev) => ({
      ...prev,
      [episodeId]: !isExpanded,
    }));

    // Fetch images if expanding and not already loaded
    if (!isExpanded && !chapterImagesMap[episodeId]) {
      await fetchChapterImages(episodeId);
    }
  };

  const handleDeleteChapterImage = async (episodeId, imageId) => {
    ...
    try {
      ...
      setChapterImagesMap((prev) => ({
        ...prev,
        [episodeId]: prev[episodeId].filter((img) => img.id !== imageId),
      }));
  ...
  const handleDragStart = (e, episodeId, imageId) => {
    setDraggedImage({ episodeId, imageId });
    e.dataTransfer.effectAllowed = 'move';
    e.dataTransfer.setData('text/html', e.target);
  };

  const handleDragOver = (e, episodeId, imageId) => {
    e.preventDefault();
    e.dataTransfer.dropEffect = 'move';
    if (draggedImage && draggedImage.episodeId === episodeId && draggedImage.imageId !== imageId) {
      setDraggedOverImage({ episodeId, imageId });
    }
  };
  ...
  const handleDrop = async (e, episodeId, imageId) => {
    ...
    const images = [...chapterImagesMap[episodeId]];
    ...
    setChapterImagesMap((prev) => ({
      ...prev,
      [episodeId]: images.map((img, index) => ({
        ...img,
        page_number: index + 1
      }))
    }));

    setIsReordering((prev) => ({ ...prev, [episodeId]: true }));

    try {
      await apiClient.reorderEpisodeVideos(episodeId, reorderedImages);
      ...
      setIsReordering((prev) => ({ ...prev, [episodeId]: false }));
      setDraggedImage(null);
      setDraggedOverImage(null);
    }
  };
